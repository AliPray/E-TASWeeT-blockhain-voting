/**
 * this test works on the smart contract interactions
 * return values
 * adding values to smart contract 
 * so they can be used in other pages
 */
...
export default function testweb3() {
    ...
    const [candidatesNumber, setCandidateNumber] = useState()
    ...
    /*
    old function for connect   //connect to wallet function 
  // async function connect() {
  //   if (typeof window.ethereum !== "undefined") {
  //     try {
  //       await ethereum.request({ method: 'eth_accounts' })
  //       setIsConnected(true);
  //       //get signer from the wallet provider ( metamask in our case)
  ...
  // }
    */
    ...
    /**
     * this function return numberof candidates and number of voters
     */
    ...
    /**
     * this function returns the candidate info by its id
     */
    async function executeCandidateInfo(){
      if (typeof window.ethereum !== "undefined"){
        const abi = ABI
        const contract = new ethers.Contract(contractAddress, abi, signer);
        try {

          const result = await contract.getCandidateInfo(id)
          var array = [];
    ...
    async function executeVoterInfo(){
      if (typeof window.ethereum !== "undefined"){
        const abi = ABI
        const contract = new ethers.Contract(contractAddress, abi, signer);
        try {

          const result1 = await contract.getVoterinfo(address)
          var array1 = [];
          for(var i=0;i<result1.length;i++){
              array1.push(result1[i].toString());
          }
          console.log(array1);
    ...
    async function executeCandidateCard(){
      if (typeof window.ethereum !== "undefined"){
        const abi = ABI
        const contract = new ethers.Contract(contractAddress, abi, signer);
        try {

          const result = await contract.registeredCandidates()
          var array = [];
          for(var i=0;i<result.length;i++){
              array.push(result[i].toString());
          }
          console.log(array);
          setCandidatesCard(array)
    ...
    async function executeElectionStatus(){
      if (typeof window.ethereum !== "undefined"){
        const abi = ABI
        const contract = new ethers.Contract(contractAddress, abi, signer);
        try {
          ...
          console.log(status)
          setElectionStatus(status.toString())
    ...
    async function executeTriggerElection(){
      if (typeof window.ethereum !== "undefined"){
        const abi = ABI
        const contract = new ethers.Contract(contractAddress, abi, signer);
        try {
          ...
          setTimeout(executeElectionStatus, 20000);
  ...
  return (
    <div>
          ...
          <button onClick={()=>executeNumbers()}>getnumber</button>
          <h1>the number of candidates: {candidatesNumber}</h1>
          <h1>the number of voters: {votersNumber}</h1>
          <p>-----------------------------------------------</p>
          <input name="candidateid"placeholder='candidate id/number' onChange={handleInputChange}></input>
          <button onClick={()=>executeCandidateInfo()}>getcandidate</button>
          <h1>candidate: {candidateinfo}</h1>
          <input name="voteraddress" onChange={handleInputChange} placeholder='voter address'></input>
          <button onClick={()=>executeVoterInfo()}>getVoter</button>
          <h1>voterInfo: {voterInfo}</h1>
          <button onClick={()=>executeCandidateCard()}>getCandidateCards</button>
          <p>{}</p>
          <button onClick={()=>executeTriggerElection()}>triggerStatus</button>
          <span>-----------</span>
          <button onClick={()=>executeElectionStatus()}>check Status</button>
          <h1>election status: {electionStatus}</h1>
          
          {/* // <div className="h-screen flex justify-center items-center">
          //     <a target="_blank" href="https://metamask.io/download/" className="text-blue-500 no-underline hover:underline hover:text-blue-700">If you haven't installed Metamask yet, PLease download from here.</a>
          //     <button type="button" onClick={() => connect()}
          //     className="text-gray-900 bg-white hover:bg-sky-100 border border-sky-200 focus:ring-4 focus:outline-none focus:ring-sky-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-sky-600 dark:bg-sky-800 dark:border-gray-700 dark:text-white dark:hover:bg-sky-700 mr-2 mb-2">
